...
export default function AcoustIDModal({ tracks, onClose }: Props) {
  ...
  const [results, setResults] = useState<AcoustResult[]>(
    tracks.map((tr) => ({ track: tr, status: "pending" }))
  );
  ...
  const runFingerprint = async () => {
    ...
    for (const r of results) {
      ...
      try {
        const data = await invoke<Record<string, unknown>>("get_acoustid_fingerprint", { path: r.track.path, apiKey });
        const res_arr = (data.results as unknown[]) ?? [];
        if (!res_arr.length) { updateResult(r.track.id, { status: "no_match" }); continue; }
        const first = res_arr[0] as Record<string, unknown>;
  ...
  const applyResult = async (r: AcoustResult) => {
    ...
    try {
      ...
      updateTrack(updated);
      setAppliedIds((prev) => new Set([...prev, r.track.id]));
      toast(t("pro.acoustid.tagApplied", { title: updated.title ?? updated.filename }), "success");
    } catch (e) {
      toast(`Error: ${e}`, "error");
    }
  };
  ...
  const applyAllMatched = () =>
    results.filter((r) => r.status === "matched" && !appliedIds.has(r.track.id)).forEach(applyResult);
      ...
      <div className="bg-[#1c1715] border border-white/[0.08] rounded-2xl w-[580px] max-h-[88vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-white/[0.06] flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-[13px] font-bold text-[#F5F5F4]">{t("pro.acoustid.title")}</h2>
            <p className="text-[10px] text-[#605A55]">{t("pro.acoustid.subtitle", { count: tracks.length })}</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-5 py-3 space-y-1">
          {results.map((r) => (
            <div key={r.track.id} className="flex items-center gap-3 px-3 py-2 rounded-lg"
              style={{
                background: r.status === "matched" ? "rgba(91,160,85,0.05)" : "rgba(255,255,255,0.02)",
                border: `1px solid ${r.status === "matched" ? "rgba(91,160,85,0.12)" : "rgba(255,255,255,0.04)"}`,
              }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                {r.status === "no_match" && <p className="text-[10px] text-[#4C4743]">{t("pro.acoustid.noMatch")}</p>}
                {r.status === "error"    && <p className="text-[10px] text-[#D95340]">{t("pro.acoustid.error")}</p>}
              </div>
              {r.status === "matched" && !appliedIds.has(r.track.id) && (
                <button onClick={() => applyResult(r)}
                  ...
                  {t("pro.acoustid.applyBtn")}
                </button>
              )}
              {appliedIds.has(r.track.id) && <span className="shrink-0 text-[10px] text-[#5BA055]">{t("pro.acoustid.applied")}</span>}
            </div>
          ))}
        </div>

        {!running && matched + noMatch + errors > 0 && (
          <div className="px-5 py-2 border-t border-white/[0.05] flex items-center gap-4 text-[10px]">
            {matched > 0 && <span className="text-[#5BA055]">{t("pro.acoustid.matchedStat", { count: matched })}</span>}
            {noMatch > 0 && <span className="text-[#605A55]">{t("pro.acoustid.noMatchStat", { count: noMatch })}</span>}
            {errors  > 0 && <span className="text-[#D95340]">{t("pro.acoustid.errorStat",   { count: errors })}</span>}
          </div>
        )}
          ...
          <div className="flex items-center gap-2">
            {matched > 1 && !running && (
              <button onClick={applyAllMatched}
                ...
                {t("pro.acoustid.applyAll", { count: matched })}
              </button>
            )}
            <button onClick={runFingerprint} disabled={running}
              ...
              {running ? t("pro.acoustid.running") : results.some((r) => r.status === "pending") ? t("pro.acoustid.runBtn") : t("pro.acoustid.restartBtn")}
            </button>
          </div>
        </div>
      </div>
